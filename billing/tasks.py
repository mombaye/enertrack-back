...
def import_status_update_task(
    ...
    try:
        ...
        df = pd.read_excel(xls, dtype=object)
        total_rows = len(df)
        del file_bytes, xls
        ...
        if missing_cols:
            dst_to_src = {dst: src for src, dst in COLUMN_MAP.items()}
            expected = [dst_to_src.get(c, c) for c in missing_cols]
            msg = (
                "Fichier non reconnu comme fichier de facturation Sénélec. "
                f"Colonnes attendues en ligne 1 de la première feuille : {expected}."
            )
            if any("energy analysis" in s.lower() for s in sheet_names):
                msg += (
                    " Ce fichier ressemble à « Analyse Marge » (analyse par site) : il ne contient "
                    "ni contrat ni numéro de facture et ne peut pas servir à mettre à jour les statuts."
                )
